        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_1.35fr]">
          <div className="space-y-6">
            <section className="rounded-[var(--radius-lg)] bg-[var(--color-secondary)] p-7 text-white shadow-[var(--shadow-card)]">
              ...
              <p className="mt-4 flex items-center gap-2 text-xs text-white/70"><LockKeyhole className="h-4 w-4" />Coupon-funded wallet only. Customer top-ups are not supported.</p>
            </section>
...
function WalletHistoryModal({ history, loading, error, filterName, page, onPageChange, onClearFilter, onClose }: { history: WalletActivityResponse | null; loading: boolean; error: string; filterName: string; page: number; onPageChange: (page: number) => void; onClearFilter: () => void; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[110] flex items-end justify-center bg-[#111827]/55 backdrop-blur-[2px] sm:items-center sm:p-5" onMouseDown={onClose}><section role="dialog" aria-modal="true" aria-labelledby="wallet-history-title" className="flex max-h-[88vh] w-full max-w-xl flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl" onMouseDown={(event) => event.stopPropagation()}><header className="flex items-start justify-between gap-4 border-b border-[#e5e9f0] px-5 py-4"><div><h2 id="wallet-history-title" className="text-lg font-extrabold text-[#172033]">{filterName ? "Coupon usage" : "Redemption history"}</h2><p className="mt-1 text-xs text-[#68748a]">{filterName || "Wallet coupons used on orders and restored after cancellation."}</p>{filterName && <button type="button" onClick={onClearFilter} className="mt-3 inline-flex h-8 items-center rounded-lg border border-[#cbd2df] px-3 text-xs font-bold text-[#34415c] hover:bg-[#f5f7fa]">Show all redemptions</button>}</div><button type="button" onClick={onClose} className="grid h-9 w-9 place-items-center rounded-full bg-[#f1f3f7] text-[#26344f]" aria-label="Close redemption history"><X className="h-5 w-5" /></button></header><div className="overflow-y-auto px-5 py-4">{loading ? <p className="py-10 text-center text-sm text-[#68748a]">Loading redemption history…</p> : error ? <p className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</p> : !history?.activity.length ? <p className="py-10 text-center text-sm text-[#68748a]">No wallet coupons have been redeemed yet.</p> : <div className="space-y-2.5">{history.activity.map((item) => { const restored = item.type === "cancellation_reversal"; return <article key={item.id} className="flex items-start gap-3 rounded-xl border border-[#e5e9f0] p-4"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-full ${restored ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-800"}`}>{restored ? <ArrowDownLeft className="h-5 w-5" /> : <ArrowUpRight className="h-5 w-5" />}</span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-[#172033]">{restored ? `Restored after Order #${item.order_id || "—"} cancellation` : `Used on Order #${item.order_id || "—"}`}</p><p className="mt-0.5 truncate text-xs text-[#68748a]">{item.coupon_name} · {item.coupon_code}</p></div><p className={`shrink-0 font-extrabold ${restored ? "text-green-700" : "text-[#172033]"}`}>{restored ? "+" : "-"}{formatCurrency(Math.abs(item.amount))}</p></div><div className="mt-2 flex items-center justify-between gap-3 text-xs text-[#68748a]"><span>{formatDateTime(item.occurred_at)}</span><span>Credit left {formatCurrency(item.credit_balance_after)}</span></div></div></article>; })}</div>}</div>{pagination && pagination.totalPages > 1 && <footer className="flex items-center justify-between border-t border-[#e5e9f0] px-5 py-3"><button type="button" disabled={page <= 1 || loading} onClick={() => onPageChange(page - 1)} className="rounded-lg border px-3 py-2 text-sm font-bold disabled:opacity-40">Previous</button><span className="text-xs text-[#68748a]">Page {pagination.page} of {pagination.totalPages}</span><button type="button" disabled={page >= pagination.totalPages || loading} onClick={() => onPageChange(page + 1)} className="rounded-lg border px-3 py-2 text-sm font-bold disabled:opacity-40">Next</button></footer>}</section></div>;
}
